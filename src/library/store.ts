...
export function parseCharacterInput(body: Record<string, unknown>, existing?: StoredCharacter): StoredCharacter {
  ...
  const species = asText(body.species, 160);
  ...
  let referenceImage = existing?.referenceImage;
  if ("referenceImage" in body) {
    ...
    if (img == null || img === "") {
      ...
      if (Buffer.byteLength(raw, "base64") > MAX_IMAGE_BYTES) {
        throw new Error("La imagen del personaje supera 2MB");
      }
      referenceImage = raw;
    } else {
      throw new Error("referenceImage debe ser base64");
    }
  }
...
export function listCharacters(userId: string): StoredCharacter[] {
  return requireUser(userId).characters ?? [];
}
