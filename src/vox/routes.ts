...
export async function registerVoxRoutes(app: FastifyInstance, redis: IORedis): Promise<void> {
  ...
  app.post("/api/v1/vox/jobs", async (request: AuthedRequest, reply) => {
    ...
    if (
      !VOX_DURATIONS.includes(durationSec as (typeof VOX_DURATIONS)[number]) &&
      ![15, 30, 60].includes(durationSec)
    ) {
    ...
    const atlasKey = resolveAtlasApiKey();
    const visualProvider = resolveStudioVisualProvider(
      typeof body.visualProvider === "string" ? body.visualProvider : undefined,
    );
    if (!atlasKey && mode === "aroll") {
      return reply.status(400).send({
        error: "A-roll necesita ATLASCLOUD_API_KEY en el servidor (video / video-worker)",
      });
    }
    const config: VoxJobConfig = {
      ...
      themes: Array.isArray(body.themes)
        ? (body.themes as string[]).slice(0, 4)
        : [...DEFAULT_BAKEOFF],
      videoModel: body.realPeople
        ? KLING_VIDEO_MODEL
        : String(body.videoModel ?? DEFAULT_VIDEO_MODEL),
      ...
      music: String(
        body.music ?? "editorial documentary, paper-texture percussion, instrumental, no vocals",
      ),
  ...
  app.patch<{ Params: { id: string } }>(
    ...
    async (request: AuthedRequest, reply) => {
      ...
      if (!body.beats?.beats?.length)
        return reply.status(400).send({ error: "beats.json inválido" });
  ...
  app.post<{ Params: { id: string } }>(
    "/api/v1/vox/jobs/:id/approve-beats",
    async (request: AuthedRequest, reply) => {
      const user = requireUser(request, reply);
      if (!user) return;
      const meta = readMeta(request.params.id);
      if (!meta || !ownerOk(meta, user.id))
        return reply.status(404).send({ error: "No encontrado" });
      if (meta.status !== "awaiting_beats")
        return reply.status(400).send({ error: "Nada que aprobar" });
      if (!readBeats(meta.id)) return reply.status(400).send({ error: "Falta beats.json" });
      if (
        resolveStudioVisualProvider(meta.config.visualProvider) !== "gflow" &&
        !resolveAtlasApiKey(meta.config.atlasKey)
      ) {
        return reply
          .status(400)
  ...
  app.post<{ Params: { id: string } }>(
    "/api/v1/vox/jobs/:id/retry-bakeoff",
    async (request: AuthedRequest, reply) => {
      const user = requireUser(request, reply);
      if (!user) return;
      const meta = readMeta(request.params.id);
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request: AuthedRequest, reply) => {
      ...
      if (meta.status !== "awaiting_style")
        return reply.status(400).send({ error: "Aún no hay bake-off" });
      ...
      if (!resolveAtlasApiKey(meta.config.atlasKey)) {
        return reply
          .status(400)
          .send({ error: "Falta ATLASCLOUD_API_KEY en el servidor (voz / video-worker)" });
      }
      beats.theme = theme;
