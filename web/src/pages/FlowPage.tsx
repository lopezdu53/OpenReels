...
function isFlowJob(job: JobSummary) {
  return job.config?.image === "gflow" || job.config?.video === "gflow";
}
...
export function FlowPage() {
  ...
  useEffect(() => {
    ...
    api.listJobs(50, 0).then(({ jobs: listed }) => {
      const films = listed.filter(isFlowJob);
  ...
  const llmList = (providers?.llm?.length ? providers.llm : FALLBACK.llm).filter((p) =>
    FALLBACK.llm.some((a) => a.key === p.key),
  );
  const ttsList = (providers?.tts?.length ? providers.tts : FALLBACK.tts).filter((p) =>
    FALLBACK.tts.some((a) => a.key === p.key),
  );
  const imageList = FALLBACK.image;
  const videoList = FALLBACK.video;
  const searchList = FALLBACK.search;
  const kokoroVoices = providers?.kokoroVoices?.length ? providers.kokoroVoices : FALLBACK_KOKORO;

  const prices = useMemo(() => {
    const base = loadPrices();
    const llm = providers?.atlasLlmModels?.find((m) => m.id === (llmModel || "deepseek-ai/deepseek-v4-flash"));
  ...
  async function generateScript() {
    ...
    setError("");
    try {
      const { script } = await api.generateFlowScript({
        idea: ideaText,
        durationMinutes,
        llm: llmProvider,
    ...
    <div className="px-4 sm:px-6 lg:px-10 py-8 lg:py-10">
      <div className="mx-auto max-w-6xl">
        <div className="mb-6 max-w-2xl">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-[2px] text-primary">YouTube 16:9 · Google Flow</p>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (next > 0 && next < 0.75) {
                    ...
                    if (!videoProvider) setVideoProvider("gflow");
                  } else {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Field label="Búsqueda web">
            <Select value={searchProvider} onValueChange={(v) => v && setSearchProvider(v)}>
              <SelectTrigger className={FIELD}><SelectValue>{labelOf(searchList, searchProvider)}</SelectValue></SelectTrigger>
              <SelectContent>
                {searchList.map((p) => (
                  <SelectItem key={p.key} value={p.key}>{p.label}</SelectItem>
                ))}
        ...
        <PipelineStep id="film-visuales" step={5} icon={ImageIcon} title="Visuales" subtitle="gflow-cli: Imagen stills + Veo I2V">
          <div className="space-y-4">
            ...
            {(imageProvider === "gflow" || videoProvider === "gflow") ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-xl border border-primary/30 bg-primary/5 p-4">
                {imageProvider === "gflow" ? (
                  <Field label="Modelo Imagen (gflow)">
                    <Select value={gflowImageModel} onValueChange={(v) => v && setGflowImageModel(v)}>
                      <SelectTrigger className={FIELD}><SelectValue /></SelectTrigger>
                      <SelectContent>
