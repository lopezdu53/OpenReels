...
- **ai_video**: AI-generated video clip animated from an AI image. Use when MOTION is the story — explosions, flowing water, rocket launches, walking, flying, transformations, dynamic action. More expensive than ai_image (~$0.30/scene vs ~$0.04/scene). Use selectively: 1-3 scenes per Short where motion truly adds value, not as a default upgrade. Set motion to "static" for ai_video scenes (the video model controls motion, not Ken Burns). **Duration limit: ai_video clips max out at 8 seconds. Keep script_lines for ai_video scenes under 18 words so voiceover fits within the clip.** Longer narration causes the video to freeze on its last frame. Between a still image and an ai_video (or back), ALWAYS use transition "crossfade" — never "none".
- **FOLLOW-CAM HERO (Film / Flow)**: the first CAST member never leaves frame. Each clip is a SPECTACLE beat (hero handles a NEW named object, or the world morphs around the body). Ban idle walks. Every 3rd beat: insert of that object or a reaction close_up. Hook scene 0 is a visual WTF.
